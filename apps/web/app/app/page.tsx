  ...
  return (
    <main className="grid gap-6">
      ...
      <section className="grid gap-3">
        <h2 className="text-xl font-black">Recent boards</h2>
          ...
          {data?.items.length ? (
            data.items.map(({ board, libraryItem }) => (
              <Link
                ...
                className="drawi-panel block p-4 transition-transform hover:-translate-y-0.5"
              >
                <p className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--ink-3)]">
                  {libraryItem.relationship}
                </p>
                <h3 className="mt-2 text-lg font-black">{board.title}</h3>
