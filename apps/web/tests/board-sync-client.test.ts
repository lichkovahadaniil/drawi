...
describe("board sync client", () => {
  ...
  it("creates encoded sync and asset urls", () => {
    expect(
      createBoardSyncConnectUrl("http://localhost:8787", "room", {
        sessionId: "session",
        accessToken: "token",
      }),
    ).toBe("ws://localhost:8787/api/connect/room?sessionId=session&accessToken=token");
    expect(
      createBoardSyncConnectUrl("http://localhost:8787", "room", {
        sessionId: "session",
        accessToken: "",
      }),
    ).toBe("ws://localhost:8787/api/connect/room?sessionId=session");
    expect(
      createBoardSyncConnectUrl("https://sync.example.com/", "room/one", {
        sessionId: "session 1",
        accessToken: "token.1",
      }),
    ).toBe("wss://sync.example.com/api/connect/room%2Fone?sessionId=session+1&accessToken=token.1");
    ...
    expect(
      createBoardAssetUrl("https://sync.example.com/", "room/one", "image 1.png", "token.1"),
    ).toBe("https://sync.example.com/api/uploads/room%2Fone/image%201.png?accessToken=token.1");
  });

  it("adds and removes access tokens from worker asset urls without touching other urls", () => {
    const assetUrl = "https://sync.example.com/api/uploads/room/image.png";
    const authorized = authorizeBoardAssetUrl(assetUrl, "token.1");

    expect(authorized).toBe(
      "https://sync.example.com/api/uploads/room/image.png?accessToken=token.1",
    );
    expect(stripBoardAssetAccessToken(authorized)).toBe(assetUrl);
    expect(authorizeBoardAssetUrl(assetUrl, "")).toBe(assetUrl);
    expect(authorizeBoardAssetUrl("https://cdn.example.com/image.png", "token")).toBe(
      "https://cdn.example.com/image.png",
    );
    expect(authorizeBoardAssetUrl("not a url", "token")).toBe("not a url");
    expect(stripBoardAssetAccessToken("data:image/png;base64,abc")).toBe(
      "data:image/png;base64,abc",
    );
    expect(stripBoardAssetAccessToken("not a url")).toBe("not a url");
  });
  ...
  it("requests a short-lived access cookie before opening the websocket", async () => {
    const fetcher = vi.fn(
      async () =>
        new Response(
          JSON.stringify({ ok: true, sessionId: "issued-session", accessToken: "access-token" }),
          { status: 200 },
        ),
    );

    await expect(ensureBoardSyncAccess("board-1", fetcher)).resolves.toEqual({
  ...
  it("fails the connection when the session cookie cannot be issued", async () => {
    ...
    await expect(ensureBoardSyncAccess("board-1", fetcher)).rejects.toThrow(
      "Collaborative session unavailable.",
    );
  });

  it("fails the connection when the session cookie response omits websocket access data", async () => {
  ...
  it("keeps the websocket uri resolver stable and side-effect free until connection time", async () => {
    const fetcher = vi.fn(
      async () =>
        new Response(JSON.stringify({ ok: true, sessionId: "issued session", accessToken: "t" }), {
          status: 200,
        }),
    );
    ...
    await expect(resolveUri()).resolves.toBe(
      "wss://sync.example.com/api/connect/room%2Fone?sessionId=issued+session&accessToken=t",
    );
    expect(fetcher).toHaveBeenCalledOnce();
  });

  it("lets tests and callers override the issued websocket session id", async () => {
    const fetcher = vi.fn(
      async () =>
        new Response(JSON.stringify({ ok: true, sessionId: "issued session", accessToken: "t" }), {
          status: 200,
    ...
    await expect(resolveUri()).resolves.toBe(
      "wss://sync.example.com/api/connect/room%2Fone?sessionId=fixed+session&accessToken=t",
    );
  });
